(**
 * $Id: dco.framework.RPCObjectImpl.pas 840 2014-05-24 06:04:58Z QXu $
 *
 * Software distributed under the License is distributed on an "AS IS" basis, WITHOUT WARRANTY OF ANY KIND, either
 * express or implied. See the License for the specific language governing rights and limitations under the License.
 *)

unit dco.framework.RPCObjectImpl;
...
begin
  ...
  try
    ...
    begin
      ...
      {$IFDEF LOGGING}
      TLogLogger.GetLogger(ClassName).Error('%s Stop receiving further messages: %s', [GetId, E.ToString]);
      {$ENDIF}
...
begin
  ...
  try
    ...
    begin
      {$IFDEF LOGGING}
      TLogLogger.GetLogger(ClassName).Error('%s Rejected unhandled request (method="%s", params=%s, id="%s")',
    ...
    try
      ...
      begin
        ...
        {$IFDEF LOGGING}
        TLogLogger.GetLogger(ClassName).Error('Unhandled exception while handling request: %s\n%s',
            [Method, E.ToString]);
        {$ENDIF}
...
begin
  ...
  try
    ...
    begin
      {$IFDEF LOGGING}
      TLogLogger.GetLogger(ClassName).Warn('%s Ignored unhandled notification (method="%s", params=%s)',
    ...
    try
      ...
      {$IFDEF LOGGING}
      on E: Exception do
        TLogLogger.GetLogger(ClassName).Error('Unhandled exception while handling notification: %s\n%s',
            [Method, E.ToString]);
      {$ENDIF}
    end;
  finally
...
begin
  ...
  try
    ...
    begin
    {$IFDEF LOGGING}
      TLogLogger.GetLogger(ClassName).Warn('%s Ignored unexpected response (result="%s", id="%s")',
    ...
    begin
    {$IFDEF LOGGING}
      TLogLogger.GetLogger(ClassName).Warn('%s Ignored response with unsupported id (result="%s", id="%s")',
          [GetId, TJson.Print(Result), Id.ToString]);
    {$ENDIF}
    end;
  end;
...
begin
  ...
  try
    ...
    begin
    {$IFDEF LOGGING}
      TLogLogger.GetLogger(ClassName).Warn('%s Ignored unexpected response (id="%s"): %s',
    ...
    begin
    {$IFDEF LOGGING}
      TLogLogger.GetLogger(ClassName).Warn('%s Ignored response with unsupported id (id="%s"): %s',
        [GetId, Id.ToString, Error.ToString]);
    {$ENDIF}
    end;
  end;
...
function TRPCObjectImpl.ExecuteAwait(const Method: string; const Params: ISuperObject): ISuperObject;
begin
  Result := ExecuteAwait(Method, Params, TTimeSpan.FromSeconds(30));
end;
...
begin
  ...
  Result := ExecuteAwait(Command.Method_, Command.Params_, Timeout);
end;

function TRPCObjectImpl.ExecuteAwait(const Method: string; const Params: ISuperObject; const Timeout: TTimeSpan): ISuperObject;
var
  ResultContainer: TResult<ISuperObject>;
  Id: TIdentifier;
  Message_: string;
...
begin
  ...
  try
    ...
    Message_ := FSerializer.EncodeRequest(Method, Params, Id);
...
begin
  ...
  Notify(Command.Method_, Command.Params_);
end;

procedure TRPCObjectImpl.Notify(const Method: string; const Params: ISuperObject);
var
  Message_: string;
begin
  assert(Method <> '');
  ...
  Message_ := FSerializer.EncodeNotification(Method, Params);
  FConnection.Write(Message_);
end;
