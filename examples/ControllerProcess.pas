...
type
  ...
  private
    ...
    FClient: TControlRemoteWindowProtocol.Iface;
...
begin
  ...
  TProcess.Fork(Application.ExeName, Format('%s %s', [PROCESS_UUID, LocalUri]));
  RPCObject := nil;
  StartTime := Now;
  ...
  FClient := TControlRemoteWindowProtocol.TClient.Create(RPCObject);
...
begin
  FClient.ResizeWindow(100, 100);
end;

procedure TForm1.Button3Click(Sender: TObject);
begin
  FClient.ResizeWindow(200, 200);
end;
...
begin
  ...
  ConfigureLogger;
  FRemotingSystem := TRemotingSystem.Create(TNamedPipeTransportImpl.Create('form1'));
